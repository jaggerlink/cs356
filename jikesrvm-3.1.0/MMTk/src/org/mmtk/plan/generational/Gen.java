...
public abstract class Gen extends StopTheWorld {
  ...
  // RaceDet: used for counting in testing configuration (not currently used)
  //public static int majorGCCount;
  
  /** The nursery space is where all new objects are allocated by default */
  // RaceDet: more virtual space
  private static final VMRequest vmRequest = USE_DISCONTIGUOUS_NURSERY ? VMRequest.create() : VMRequest.create(64, true);
    ...
    if (phaseId == PREPARE) {
      ...
      if (traceFullHeap()){
        if (gcFullHeap) {
          ...
          // RaceDet: used for special configuration (not currently used)
          //majorGCCount++;
