...
public final class SpecializedScanMethod extends SpecializedMethod implements SizeConstants, JavaHeaderConstants {
  ...
  private static int getPattern(RVMType type) {
    ...
    if (type.isArrayType()) {
      ...
      // RaceDet: if we were supporting direct field metadata for arrays, we'd need to do something special here (like the commented-out code)
      // Note we can't actually return FALLBACK_PATTERN since the code assumes that's an array
      /*
