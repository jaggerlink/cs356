...
import org.jikesrvm.rd.Barriers;
...
public abstract class FieldLayout implements SizeConstants {
  ...
  */
  ...
  protected void updateClass(RVMClass klass, FieldLayoutContext fieldLayout) {
    ...
    // RaceDet: set number of indirect metadata fields
    klass.setIndirectMetadataFields(fieldLayout.nextFieldNumber);
  }
  ...
  /** RaceDet: set metadata offset */
  protected void setMetadataOffset(RVMClass klass, RVMField field, Offset offset) {
  ...
  /** RaceDet: make room for per-field metadata */
  protected void layoutFieldMetadata(FieldLayoutContext layout, RVMClass klass, RVMField field) {
    boolean direct = false;
    boolean indirect = false;
    if (RaceDet.addDirectFieldMetadata(field)) {
      direct = layoutDirectFieldMetadata(layout, klass, field);
    }
    if (RaceDet.addIndirectFieldMetadata(field)) {
      indirect = layoutIndirectMetadata(layout, klass, field);
    }
    if (VM.VerifyAssertions) { VM._assert(!(direct && indirect)); }
  }

  protected boolean layoutDirectFieldMetadata(FieldLayoutContext layout, RVMClass klass, RVMField field) {
    if (!field.hasMetadataOffset()) {
      int numFields = Barriers.getNumFields(field);
      Offset metadataOffset;
      if (numFields > 1) {
        metadataOffset = Offset.fromIntSignExtend(layout.nextOffsetUnaligned(Barriers.FIELD_SIZE * numFields));
      } else {
        metadataOffset = Offset.fromIntSignExtend(layout.nextOffset(Barriers.FIELD_SIZE, true));
      }
      setMetadataOffset(klass, field, metadataOffset);
      return true;
    }
    return false;
  }

  protected boolean layoutIndirectMetadata(FieldLayoutContext layout, RVMClass klass, RVMField field) {
    if (!field.hasFieldNumber()) {
      field.setFieldNumber(layout.nextFieldNumber);
      int numMetadataFields = Barriers.getNumFields(field);
      if (VM.VerifyAssertions) { VM._assert(numMetadataFields == 1); }
      layout.nextFieldNumber++;
      return true;
    }
    return false;
  }
