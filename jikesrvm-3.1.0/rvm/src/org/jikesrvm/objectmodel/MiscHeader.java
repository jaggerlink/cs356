...
public final class MiscHeader implements Constants, MiscHeaderConstants {
  ...
  public static final Offset LOCK_VC_OFFSET = MISC_HEADER_START;
  public static final Offset FIELD_METADATA_OFFSET = LOCK_VC_OFFSET.plus(LOCK_VC_BYTES);
  public static final Offset OBJECT_COUNT_OFFSET = FIELD_METADATA_OFFSET.plus(FIELD_METADATA_BYTES);
  public static final Offset ALLOC_SITE_OFFSET = OBJECT_COUNT_OFFSET.plus(OBJECT_COUNT_BYTES);
  public static final Offset ALLOC_SITE_COUNT_OFFSET = ALLOC_SITE_OFFSET.plus(ALLOC_SITE_BYTES);
   ...
   */
  // RaceDet: added inline directive and commented out a lot since otherwise it burdens compiler
  @Inline
  @Uninterruptible
  public static void initializeHeader(Object obj, TIB tib, int size, boolean isScalar) {
    ...
    */
    ...
    // RaceDet: track allocation sites
    /*
    if (RaceDet.trackAllocSites()) {
      int kpcc = RVMThread.getCurrentThread().pccStackTrace.computePCC(3, false);
      boolean success = LockVC.updateAllocSites(kpcc);
      if (success) {
        int count = LockVC.getAllocSiteCount(kpcc);
        LockVC.setAllocSite(ObjectReference.fromObject(ref), kpcc, count);
      }
    }
    */
  }
  ...
  public static void initializeHeader(BootImageInterface bootImage, Address ref, TIB tib, int size,
    ...
    // RaceDet: trace pointers to indirect metadata during boot image scan
    if (LOCK_VC_BYTES > 0) {
      bootImage.setAddressWord(ref.plus(LOCK_VC_OFFSET), Word.zero(), true, true);
    }
    if (FIELD_METADATA_BYTES > 0) {
      bootImage.setAddressWord(ref.plus(FIELD_METADATA_OFFSET), Word.zero(), true, true);
    }
  }
