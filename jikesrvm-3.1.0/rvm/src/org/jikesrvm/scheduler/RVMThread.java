...
import org.jikesrvm.rd.Algo;
import org.jikesrvm.rd.RaceDet;
import org.jikesrvm.rd.Sampling;
import org.jikesrvm.rd.Stats;
import org.jikesrvm.rd.VC;
...
public class RVMThread extends ThreadContext {
  ...
  // RaceDet: thread ID, vector clock, and epoch
  
  /** Get this thread's ID according to race detection */
  @Inline
  public final int getRaceDetID() {
    // RaceDet: LATER: doesn't reuse thread IDs
    return raceDetID;
    //return threadSlot - 1;
  }
  private final int raceDetID;
  private static int nextRaceDetID = 0;
  
  /** Current clock for this thread, for fast checks.  Needs to be updated when vector clock is updated. */
  public int myClock;
  
  /** Vector clock for this thread.   When updated, clock needs to be updated as well. */
  public VC vectorClock;
  
  /** Number of synchronization operations on this thread (like myClock, but incremented during non-sampling periods too). */
  public int numIncrements;
  
  @Inline
  public final void incrementClock() {
    if (Sampling.incClocks()) {
      incrementClockSlowPath();
    }
    numIncrements++;
  }
  
  @NoInline
  private final void incrementClockSlowPath() {
    int tid = getRaceDetID();
    vectorClock = vectorClock.incrementClock(tid);
    myClock = vectorClock.getClock(tid);
  }
  ...
  public RVMThread(byte[] stack, Thread thread, String name, boolean daemon,
    ...
    if (!VM.runningVM) {
      ...
      // RaceDet: not an applicable thread
      this.raceDetID = -1;
      ...
      // RaceDet: get a thread ID that'll be used by race detection
      if (!isSystemThread()) {
        acctLock.lockNoHandshake();
        raceDetID = nextRaceDetID++;
        acctLock.unlock();
        this.vectorClock = new VC(getRaceDetID());
        if (RaceDet.verbosity() >= 2) {
          VM.sysWriteln("Creating new thread with raceDetID = ", getRaceDetID(), " and threadSlot = ", getThreadSlot());
        }
      } else {
        raceDetID = -1;
        this.vectorClock = null;
  ...
  public final void start() {
    ...
    if (RaceDet.syncOps() && RaceDet.isApplicableThread(this)) {
      Algo.fork(RVMThread.getCurrentThread(), this);
    }
  ...
  public final void terminate() {
    ...
    // RaceDet: account for increments performed by dying thread
    if (RaceDet.syncOps() && RaceDet.useSampling() && RaceDet.isApplicableThread(this)) {
      Sampling.deadThreadIncrements += this.numIncrements;
    }
    // RaceDet: keep track of how many threads are live
    if (RaceDet.stats() && RaceDet.isApplicableThread(this)) {
      Stats.maxLiveThreads.dec();
    }
