...
public final class RVMClass extends RVMType implements Constants, ClassLoaderConstants {
  ...
  /** RaceDet: number of indirect metadata fields */
  private int indirectMetadataFields;
  ...
  @Uninterruptible
  public void setIndirectMetadataFields(int numFields) {
    indirectMetadataFields = numFields;
  }

  /** RaceDet: get number of indirect metadata fields */
  @Uninterruptible
  public int getIndirectMetadataFields() {
    return indirectMetadataFields;
  }
  ...
  public synchronized void resolve() {
    ...
    for (RVMField field : staticFields) {
      ...
      // RaceDet: add per-field metadata for static fields
      if (RaceDet.addDirectFieldMetadata(field)) {
        if (!field.hasMetadataOffset()) {
          int numFields = Barriers.getNumFields(field);
          Offset metadataOffset = Statics.allocateReferenceSlots(true, numFields);
    ...
    for (RVMField field : instanceFields) {
      ...
      // RaceDet: count per-field metadata, even for untraced fields
      if (RaceDet.addDirectFieldMetadata(field)) {
        int numFields = Barriers.getNumFields(field);
        for (int whichOffset = 0; whichOffset < numFields; whichOffset++) {
          if (Barriers.isReference(field, whichOffset)) {
            referenceFieldCount += 1;
          }
        }
      }
    ...
    if (typeRef.isRuntimeTable()) {
      ...
      for (RVMField field : instanceFields) {
        ...
        if (RaceDet.addDirectFieldMetadata(field)) {
          ...
          for (int whichOffset = 0; whichOffset < numFields; whichOffset++) {
            if (Barriers.isReference(field, whichOffset)) {
              referenceOffsets[j++] = field.getMetadataOffset().toInt() + whichOffset * Barriers.FIELD_SIZE;
            }
          }
        }
      }
  ...
  public synchronized void initialize() throws ExceptionInInitializerError {
    ...
    // RaceDet: LATER: deal with this -- maybe just ignore (<clinit>, any) races?
    // RaceDet: create a happens-before edge between static initializers and class variable uses
