...
public final class MemoryManager implements HeapLayoutConstants, Constants {
  ...
  */
  ...
  /** RaceDet: tell the GC that we're in a barrier */
  public static void setAllocatingInBarrier(boolean inBarrier) {
    // RaceDet: LATER: instead of this, can we make sure that yieldpoints call poll() and thus avoid launching GC?
    if (inBarrier) {
    ...
    if (VM.VerifyAssertions) {
      VM._assert(Selected.Mutator.get().allocatingInBarrier >= 0);
    }
    Selected.Mutator.get().allocatingInBarrier += (inBarrier ? 1 : -1);
    if (VM.VerifyAssertions) {
      VM._assert(Selected.Mutator.get().allocatingInBarrier >= 0);
    }
    
    // important to check for async collection
    if (Selected.Mutator.get().allocatingInBarrier == 0) {
  ...
  // RaceDet: needed for statistics (not currently used)
  public static final int getMajorCollectionCount() {
    //return Gen.majorGCCount;
    return 0;
  }
