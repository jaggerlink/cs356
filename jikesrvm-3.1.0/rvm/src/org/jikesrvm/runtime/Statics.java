...
public class Statics implements Constants {
  ...
  /** RaceDet: for allocating multiple contiguous slots */
  public static synchronized Offset allocateReferenceSlots(boolean field, int numSlots) {
    Offset first = allocateReferenceSlot(field);
    for (int i = 1; i < numSlots; i++) {
      Offset next = allocateReferenceSlot(field);
      if (VM.VerifyAssertions) { VM._assert(next.EQ(first.plus(i * BYTES_IN_ADDRESS))); }
    }
    return first;
  }
