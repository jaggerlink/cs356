...
public class GenerateInterfaceDeclarations {
  ...
  static void emitCDeclarationsForJavaType(String Cname, RVMClass cls) {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      // RaceDet: handle extra metadata fields
      if (RaceDet.shouldAddSomeFieldMetadata(field)) {
        for (int j = 0; current.sLT(offset); current = current.plus(4), j++) {
          pln("  uint32_t    padding" + i + "_" + j + ";");
        }
      }
