...
public abstract class BaselineCompilerImpl extends BaselineCompiler implements BaselineConstants, SizeConstants {
  ...
  protected final void emit_iaload() {
    ...
    // RaceDet: insert read barrier
    insertArrayBarrier(true, ONE_SLOT, TypeReference.IntArray);
  ...
  protected final void emit_aaload() {
    ...
    // RaceDet: insert read barrier
    insertArrayBarrier(true, ONE_SLOT, TypeReference.JavaLangObjectArray);
  ...
  protected final void emit_caload() {
    ...
    // RaceDet: insert read barrier
    insertArrayBarrier(true, ONE_SLOT, TypeReference.CharArray);
  ...
  protected final void emit_saload() {
    ...
    // RaceDet: insert read barrier
    insertArrayBarrier(true, ONE_SLOT, TypeReference.ShortArray);
  ...
  protected final void emit_baload() {
    ...
    // RaceDet: insert read barrier
    insertArrayBarrier(true, ONE_SLOT, TypeReference.ByteArray);
  ...
  protected final void emit_laload() {
    ...
    // RaceDet: insert read barrier
    insertArrayBarrier(true, ONE_SLOT, TypeReference.LongArray);
  ...
  protected final void emit_iastore() {
    ...
    // RaceDet: insert write barrier
    insertArrayBarrier(false, TWO_SLOTS, TypeReference.IntArray);
  ...
  protected final void emit_aastore() {
    ...
    // RaceDet: insert write barrier
    insertArrayBarrier(false, TWO_SLOTS, TypeReference.JavaLangObjectArray);
  ...
  protected final void emit_castore() {
    ...
    // RaceDet: insert write barrier
    insertArrayBarrier(false, TWO_SLOTS, TypeReference.CharArray);
  ...
  protected final void emit_bastore() {
    ...
    // RaceDet: insert write barrier
    insertArrayBarrier(false, TWO_SLOTS, TypeReference.ByteArray);
  ...
  protected final void emit_lastore() {
    ...
    // RaceDet: insert write barrier
    insertArrayBarrier(false, THREE_SLOTS, TypeReference.LongArray);
  ...
  protected final void emit_unresolved_getstatic(FieldReference fieldRef) {
    ...
    // RaceDet: static read barrier
    insertStaticBarrierUnresolved(true, T0, fieldRef);
  ...
  protected final void emit_resolved_getstatic(FieldReference fieldRef) {
    ...
    // RaceDet: static read barrier
    insertStaticBarrierResolved(true, field.isVolatile(), field);
  ...
  protected final void emit_unresolved_putstatic(FieldReference fieldRef) {
    ...
    // RaceDet: static write barrier
    insertStaticBarrierUnresolved(false, T0, fieldRef);
  ...
  protected final void emit_resolved_putstatic(FieldReference fieldRef) {
    ...
    // RaceDet: static write barrier
    insertStaticBarrierResolved(false, field.isVolatile(), field);
  ...
  protected final void emit_unresolved_getfield(FieldReference fieldRef) {
    ...
    // RaceDet: field read barrier
    insertFieldBarrierUnresolved(true, ONE_SLOT, T0, fieldRef);
  ...
  protected final void emit_resolved_getfield(FieldReference fieldRef) {
    ...
    // RaceDet: field read barrier
    insertFieldBarrierResolved(true, field.isVolatile(), NO_SLOT, field);
  ...
  protected final void emit_unresolved_putfield(FieldReference fieldRef) {
    ...
    // RaceDet: field write barrier
    insertFieldBarrierUnresolved(false, fieldRef.getSize() == BYTES_IN_LONG ? THREE_SLOTS : TWO_SLOTS, T0, fieldRef);
  ...
  protected final void emit_resolved_putfield(FieldReference fieldRef) {
    ...
    // RaceDet: field write barrier
    insertFieldBarrierResolved(false, field.isVolatile(), fieldRef.getSize() == BYTES_IN_LONG ? TWO_SLOTS : ONE_SLOT, field);
  ...
  protected final void emit_monitorenter() {
    ...
    genNullCheck(asm, T0);
    genParameterRegisterLoad(asm, 1);      // pass 1 parameter word
    // RaceDet: decide which version to use
    if (RaceDet.instrumentSyncOps(method)) {
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.lockMethod.getOffset()));
    } else {
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.lockWithoutRaceDetInstrMethod.getOffset()));
    }
  }
  ...
  protected final void emit_monitorexit() {
    ...
    if (RaceDet.instrumentSyncOps(method)) {
      ...
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.unlockWithoutRaceDetInstrMethod.getOffset()));
    }
  }
  ...
  private void genPrologue() {
    ...
    if (klass.hasBridgeFromNativeAnnotation()) {
      ...
      if (RaceDet.liteRace() && RaceDet.liteRaceMethod()) {
        if (RaceDet.addSomeBarriers(method)) {
          asm.emitPUSH_Imm(method.getId());
          genParameterRegisterLoad(asm, 1);
          asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.rdLiteRaceMethodSampleMethod.getOffset()));
        }
      }
    }
  }
  ...
  /** RaceDet: barrier for resolved static fields */
  final void insertStaticBarrierResolved(boolean isRead, boolean isVolatile, RVMField field) {
    NormalMethod barrierMethod = RaceDet.getAlgoBarrier(method, isRead, true, true, isVolatile, true);
    if (barrierMethod != null && RaceDet.shouldAddSomeFieldMetadata(field)) {
      //asm.emitPUSH_Imm(Magic.getTocPointer().toInt());
      asm.emitPUSH_Imm(field.getMetadataOffset().toInt()); // push field metadata offset
      if (!isVolatile) {
        int siteID = Site.getSite(method, biStart, isRead, true, field.getType());
        asm.emitPUSH_Imm(siteID);
        //asm.emitPUSH_Imm(ObjectReference.fromObject(field.getDeclaringClass()).toAddress().toInt());
      }
      genParameterRegisterLoad(asm, isVolatile ? 1 : 2);
      asm.emitCALL_Abs(Magic.getTocPointer().plus(barrierMethod.getOffset()));
    }
  }

  /** RaceDet: barrier for unresolved static fields */
  final void insertStaticBarrierUnresolved(boolean isRead, GPR reg, FieldReference fieldRef) {
    NormalMethod barrierMethod = RaceDet.getAlgoBarrier(method, isRead, true, false, false, true);
    if (barrierMethod != null && RaceDet.mightHaveFieldMetadata(fieldRef, true)) {
      asm.emitPUSH_Reg(reg); // save value on the stack; also update where we'll find the reference
      //asm.emitPUSH_Imm(Magic.getTocPointer().toInt()); // push JTOC
      asm.emitPUSH_Imm(fieldRef.getId()); // push field ID
      int siteID = Site.getSite(method, biStart, isRead, true, fieldRef.getFieldContentsType());
      asm.emitPUSH_Imm(siteID);
  ...
  /** RaceDet: barrier for resolved fields */
  final void insertFieldBarrierResolved(boolean isRead, boolean isVolatile, Offset numSlots, RVMField field) {
    NormalMethod barrierMethod = RaceDet.getAlgoBarrier(method, isRead, true, true, isVolatile, false);
    if (barrierMethod != null && RaceDet.shouldAddSomeFieldMetadata(field)) {
      asm.emitPUSH_RegDisp(SP, numSlots); // push object reference
      if (isVolatile) {
        asm.emitPUSH_Imm(field.getMetadataOffset().toInt()); // push direct metadata offset
      } else {
        asm.emitPUSH_Imm(field.getFieldNumber()); // push indirect field number
        int siteID = Site.getSite(method, biStart, isRead, false, field.getType());
        asm.emitPUSH_Imm(siteID);
      }
      genParameterRegisterLoad(asm, isVolatile ? 2 : 3);
      asm.emitCALL_Abs(Magic.getTocPointer().plus(barrierMethod.getOffset()));
    }
  }

  /** RaceDet: barrier for unresolved fields */
  final void insertFieldBarrierUnresolved(boolean isRead, Offset numSlots, GPR reg, FieldReference fieldRef) {
    NormalMethod barrierMethod = RaceDet.getAlgoBarrier(method, isRead, true, false, false, false);
    if (barrierMethod != null && RaceDet.mightHaveFieldMetadata(fieldRef, false)) {
      asm.emitPUSH_Reg(reg); // save value on the stack; also update where we'll find the reference
      asm.emitPUSH_RegDisp(SP, numSlots); // push object reference
      asm.emitPUSH_Imm(fieldRef.getId()); // push field ID
      int siteID = Site.getSite(method, biStart, isRead, false, fieldRef.getFieldContentsType());
      asm.emitPUSH_Imm(siteID);
      genParameterRegisterLoad(asm, 3);
      asm.emitCALL_Abs(Magic.getTocPointer().plus(barrierMethod.getOffset()));
      asm.emitPOP_Reg(reg);
  ...
  /** RaceDet: barrier for arrays */
  @Inline
  private final void insertArrayBarrier(boolean isRead, Offset numSlots, TypeReference type) {
    NormalMethod barrierMethod = RaceDet.getAlgoBarrier(method, isRead, false, true, false, false);
    if (barrierMethod != null) {
      ...
      //asm.emitPUSH_Imm(FieldLayout.log2(type.getArrayElementType().getMemoryBytes())); // push log of element size
      int siteID = Site.getSite(method, biStart, isRead, false, type);
      asm.emitPUSH_Imm(siteID); // push site ID
      genParameterRegisterLoad(asm, 3);
      asm.emitCALL_Abs(Magic.getTocPointer().plus(barrierMethod.getOffset()));
    }
  }
