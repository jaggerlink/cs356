  ...
  public void run() {
    ...
    for (int count = 0; ; count++) {
      ...
      if (gcOrdinal == GC_ORDINAL_BASE) {
        ...
        // RaceDet: decide here whether to turn sampling on and off
        if (RaceDet.useSampling()) {
          boolean incClocksBefore = Sampling.incClocks();
          Sampling.samplingDecision(false);
          boolean incClocksAfter = Sampling.incClocks();
          // if start of sampling period, then increment each clock for correctness
          if (!incClocksBefore && incClocksAfter) {
            for (int i = 0; i < RVMThread.numThreads; i++) {
              RVMThread thread = RVMThread.threads[i];
              if (RaceDet.isApplicableThread(thread) && thread.isAlive()) {
                thread.incrementClock();
              }
            }
          }
        }
