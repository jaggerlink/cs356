...
public final class RVMField extends RVMMember {
  ...
  /** RaceDet: the offset of direct per-field metadata */
  private Offset metadataOffset = Offset.fromIntSignExtend(NO_OFFSET);
  
  /** RaceDet: the number (0, 1, ...) for indirect per-field metadata */
  ...
  // RaceDet: metadata offsets and field numbers
  ...
  public final int getFieldNumber() {
    if (VM.VerifyAssertions) { VM._assert(hasFieldNumber()); }
    return fieldNumber;
  }

  @Uninterruptible
  @Inline
  public final void setFieldNumber(int number) {
    fieldNumber = number;
  }
