...
public final class RVMArray extends RVMType implements Constants, ClassLoaderConstants {
  ...
  public int getInstanceSize(int numelts) {
    // RaceDet: not supporting direct per-field metadata for arrays, but commented-out code shows where we'd do it
    /*
    if (RaceDet.fieldMetadata()) {
      return ObjectModel.computeArrayHeaderSize(this) + (numelts << getLogElementSize()) + (numelts << (FieldMetadata.LOG_WORDS_OF_METADATA + LOG_BYTES_IN_WORD));
    } else*/ {
