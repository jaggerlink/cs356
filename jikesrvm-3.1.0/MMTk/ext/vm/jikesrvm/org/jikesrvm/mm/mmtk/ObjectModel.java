...
@Uninterruptible public final class ObjectModel extends org.mmtk.vm.ObjectModel implements org.mmtk.utility.Constants,
  ...
  /** RaceDet: trace an object's race detection metadata */
  @Inline
  public void traceMetadata(ObjectReference object, TraceLocal trace) {
    if (MiscHeader.LOCK_VC_BYTES > 0) {
      Address slot = object.toAddress().plus(MiscHeader.LOCK_VC_OFFSET);
      trace.processEdge(object, slot);
    }
    if (MiscHeader.FIELD_METADATA_BYTES > 0) {
      Address slot = object.toAddress().plus(MiscHeader.FIELD_METADATA_OFFSET);
      trace.processEdge(object, slot);
    }
  }
}
