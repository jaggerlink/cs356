...
public final class ExpandRuntimeServices extends CompilerPhase {
   ...
   */
  public void perform(final IR ir) {
    ir.gc.resync(); // resync generation context -- yuck...
    
    // RaceDet: do read/write barrier instrumentation here
    RaceDetInstr raceDetInstr = new RaceDetInstr(ir);
    raceDetInstr.perform();
    ...
    for (Instruction inst = ir.firstInstructionInCodeOrder(); inst != null; inst = next) {
      ...
      switch (opcode) {
        ...
        // RaceDet: insert instrumented versions of lock and unlock methods
        ...
        case MONITORENTER_opcode: {
          ...
          if (refType != null && !refType.getThinLockOffset().isMax()) {
            RVMMethod target;
            if (RaceDet.instrumentSyncOps(inst.position.getMethod())) {
              target = Entrypoints.inlineLockMethod;
            } else {
              target = Entrypoints.inlineLockWithoutRaceDetInstrMethod;
            }
            Call.mutate2(inst,
                         CALL,
                         null,
            ...
            if (RaceDet.instrumentSyncOps(inst.position.getMethod())) {
              ...
              target = Entrypoints.lockWithoutRaceDetInstrMethod;
            }
            Call.mutate1(inst,
                         CALL,
                         null,
        ...
        case MONITOREXIT_opcode: {
          ...
          if (refType != null && !refType.getThinLockOffset().isMax()) {
            ...
            if (RaceDet.instrumentSyncOps(inst.position.getMethod())) {
              ...
              target = Entrypoints.inlineUnlockWithoutRaceDetInstrMethod;
            }
            Call.mutate2(inst,
                         CALL,
                         null,
            ...
            if (RaceDet.instrumentSyncOps(inst.position.getMethod())) {
              ...
              target = Entrypoints.unlockWithoutRaceDetInstrMethod;
            }
            Call.mutate1(inst,
                         CALL,
                         null,
